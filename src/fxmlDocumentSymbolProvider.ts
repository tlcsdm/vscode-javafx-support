...
export class FxmlDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
    ...
    provideDocumentSymbols(
        ...
        const stack: { symbol: vscode.DocumentSymbol; children: vscode.DocumentSymbol[] }[] = [];
        ...
        while ((match = tagRegex.exec(text)) !== null) {
            ...
            if (isClosing) {
                // Closing tag: pop the stack and finalize the symbol range
                for (let i = stack.length - 1; i >= 0; i--) {
                    if (stack[i].symbol.name === tagName) {
                ...
                if (isSelfClosing) {
                    ...
                    stack.push({ symbol, children: symbol.children });
                }
            }
        }
